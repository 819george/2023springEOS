...
#define GPIO_21 (21)  //binary 1
#define GPIO_16 (16)  //binary 2
#define GPIO_12 (12)  //binary 4
#define GPIO_1 (1)    //binary 8
dev_t dev = 0;                   // 裝置編號
 ...
 const char __user *buf, size_t len, loff_t *off)
{
 ...
 for(i=0;i<10;i++){
  for(n=0;n<4;n++){
   binary[n]=(rec_buf[i]>>n)&1; // binary[0] 放的是二進位最低位 binary[3]放最高位
  }
  gpio_set_value(GPIO_21, binary[0]);
  gpio_set_value(GPIO_16, binary[1]);
  gpio_set_value(GPIO_12, binary[2]);
  gpio_set_value(GPIO_1,  binary[3]);
  mdelay(1000);
 }
 /*
